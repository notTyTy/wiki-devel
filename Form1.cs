    ...
    public partial class Form1 : Form
    {
        ...
        private void EditBtn_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                dataSet[nameIndex, 0] = textInfo.ToTitleCase(nameTextbox.Text);
                dataSet[nameIndex, 1] = textInfo.ToTitleCase(categoryTextbox.Text);
                dataSet[nameIndex, 2] = textInfo.ToTitleCase(structureTextbox.Text);
        ...
        {
            using SaveFileDialog saveFile = new(); // Opens windows directory
        ...
        private void Swap(int row) // Simple swap method to sort strings alphabetically
        {
            ...
            {
                (dataSet[row + 1, i], dataSet[row, i]) = (dataSet[row, i], dataSet[row + 1, i]);
            }
        }
        private void RefreshData() // Loops through the array updating the listview with updated array data
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                if (!String.IsNullOrEmpty(dataSet[i, 0]))
                {
                    ListViewItem item = new(dataSet[i, 0]);
